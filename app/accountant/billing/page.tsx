...
export default function AccountantBillingPage() {
  ...
  const filtered = invoices.filter((i: any) => {
    const matchesSearch = i.invoiceCode?.toLowerCase().includes(search.toLowerCase()) || i.service?.toLowerCase().includes(search.toLowerCase())
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          className="px-3 py-2 rounded-lg bg-[--surface] border border-[--border] text-[13px]">
